...
class HorizontalContainer (LayoutContainer):
    def __init__(self, width: int, height: int, capacity: int, padding: tuple, background_color: tuple, item_gutters: tuple,
        ...
        self._layout_logic = HorizontalLayoutLogic(self)


class HorizontalLayoutLogic(LayoutLogic):
    def __init__(self, container: HorizontalContainer):
        self._con = container
    ...
    def get_max_width_for_each_item(self) -> int:
        ...
        if (total_items == 0):
            return self._con.get_max_drawable_width()
        gutter_horizontal, gutter_vertical = self._con.get_item_gutters()
        max_width_for_each_item = (self._con.get_max_drawable_width() - (
                (total_items - 1) * gutter_vertical)) / total_items
        return int(max_width_for_each_item)
    ...
    def get_layout_coordinates(self) -> List:
        ...
        # loop through each item and calculate the coordinates for each item
        for item in self._con.get_items_copy():
            ...
            # update the x coordinate for the next item
            gutter_horizontal, gutter_vertical = self._con.get_item_gutters()
            x_cell_origin = x_cell_origin + max_width_for_each_item + gutter_vertical

        return coordinates

    def check_dimensions_when_adding_item(self):
        minimum_needed_width = self.get_minimum_layout_width()
        if minimum_needed_width > self._con.get_width():
